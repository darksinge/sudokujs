...
exports.getNakedCellsWith = ({ cells, sharedCandidates = 1, cellCount = null }) => {
  ...
  const results = []
  const combos = exports.combinations(exports.getCandidates(included), sharedCandidates)
 ...
 */
exports.getSubsetCellsWith = function * ({ cells, sharedCandidates = 1, cellCount }) {
  const unsolved = exports.getUnsolvedCells(cells)
    .filter(cell => cell.candidates.candidates.length >= sharedCandidates)

  const results = []
  const combos = exports.combinations(exports.getCandidates(unsolved), sharedCandidates)
  for (const combo of combos) {

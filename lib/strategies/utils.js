...
exports.combinations = (values, k) => {
  const combinationsRecursive = (values, k) => {
    ...
    for (let i = 0; i < values.length - k + 1; i++) {
      ...
      const tail = exports.combinations(values.slice(i + 1), k - 1)
 ...
 * @returns {string[]}
 */
exports.getCandidatesUniq = (cells, excluding) =>
  _.uniq(exports.getCandidates(cells, excluding))
 ...
 */
exports.getNakedCellsWith = ({
  cells,
  sharedCandidates = 1,
  cellCount = null,
}) => {
  const unsolved = cells.filter((cell) => cell.isEmpty())
  const included = unsolved.filter(
    (cell) => !(cell.candidates.length() < sharedCandidates),
  )

  const results = []
  const combos = exports.combinations(
    exports.getCandidates(included),
    sharedCandidates,
  )
  for (const combo of combos) {
    combo.sort()

    const cells = included
      .filter((cell) =>
        cell.candidates.candidates.every((c) => combo.includes(c)),
      )
      .filter((cell) => {
 ...
 */
exports.getSubsetCellsWith = function* ({
  cells,
  sharedCandidates = 1,
  cellCount,
}) {
  const unsolved = exports
    .getUnsolvedCells(cells)
    .filter((cell) => cell.candidates.candidates.length >= sharedCandidates)

  const results = []
  const combos = exports.combinations(
    exports.getCandidates(unsolved),
    sharedCandidates,
  )
  for (const combo of combos) {
    const cells = unsolved.filter((cell) =>
      combo.every((c) => cell.candidates.has(c)),
    )

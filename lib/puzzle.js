...
class Puzzle {
  ...
  rows() {
    return _.range(0, this.size).map((i) => this.rowAt(i))
  }

  cols() {
    return _.range(0, this.size).map((i) => this.colAt(i))
  }

  boxes() {
    return _.range(0, this.rank).flatMap((x) =>
      _.range(0, this.rank).map((y) => this.boxAt({ x, y })),
    )
  }
  ...
  isSolved() {
    this.validate()
    return this.cells.flatMap((row) => row).every((cell) => cell.hasEntry())
  }
